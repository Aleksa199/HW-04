...
def post_create_pet_simple_with_valid_data(self, auth_key: json, name: str, animal_type: str, age:str) -> json:
    ...
    def add_pets_photo(self, auth_key, pet_id, pet_photo):
        """Метод добавляет фото к существующему питомцу"""
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'images/wolf.jpg')
            })
        headers = {'auth_key': auth_key['key'], 'Content-Type': data.content_type}
